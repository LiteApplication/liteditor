...
export const useAuthStore = defineStore("auth", () => {
  ...
  const userOrgs = ref<Array<{ login: string }>>([]);
  ...
  async function fetchUser() {
    ...
    // Fetch and cache user organizations
    const orgsRes = await githubFetch("/user/orgs?per_page=100");
    if (orgsRes.ok) {
      userOrgs.value = (await orgsRes.json()) as Array<{ login: string }>;
    } else {
      userOrgs.value = [];
    }
  }
